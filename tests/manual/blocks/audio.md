...
carrier) plus an always-present caption. `controls` is always emitted — GB
parity; autoplay, loop and preload are island-canonical settings.

## Checks

- [ ] The first audio renders a player with controls; clicking it selects the block, breadcrumb reads Document › Audio.
- [ ] There is no "controls" toggle in the sidebar — the player always shows controls (GB parity).
