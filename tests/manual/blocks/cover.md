...
children slot for nested content. `dimRatio`, `minHeight`, `contentPosition`
and `hasParallax` are island-canonical; the overlay opacity and slot flex
...
- [ ] Sidebar for the first cover shows "Overlay opacity" 50, "Content position" center center, "Fixed background (parallax)" OFF.
...
```html
<div data-pb-block="cover" class="relative isolate flex min-h-[430px] overflow-hidden p-4">
  <img
    data-pb-image="image"
  ...
  >
    <h2 data-pb-block="heading" data-pb-tag="level" data-pb-text="text">Hero title</h2>
